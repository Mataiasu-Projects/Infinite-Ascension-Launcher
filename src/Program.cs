...
internal sealed class LauncherForm : Form
{
    ...
    private string TempRoot => Path.Combine(Path.GetTempPath(), "InfiniteAscensionUpdate");
    ...
    private void BuildUi()
    {
        ...
        var title = new Label { Text = "INFINITE ASCENSION", AutoSize = true, Font = new Font("Segoe UI Semibold", 18F), Location = new Point(28, 19) };
        serverLabel.AutoSize = true;
        serverLabel.Location = new Point(360, 26);
        buildLabel.AutoSize = true;
        buildLabel.Location = new Point(360, 47);
        top.Controls.Add(title);
        top.Controls.Add(serverLabel);
        top.Controls.Add(buildLabel);

        nav.Dock = DockStyle.Left;
        nav.Width = 190;
        nav.BackColor = Color.FromArgb(18, 17, 27);
        AddNavButton("Home", 22);
        AddNavButton("Settings", 70);
        AddNavButton("Logs", 118);

        content.Dock = DockStyle.Fill;
        content.Padding = new Padding(26);
        home.Dock = DockStyle.Fill;
        content.Controls.Add(home);

        var hero = new Panel { Dock = DockStyle.Top, Height = 150, BackColor = Color.FromArgb(28, 25, 43) };
        var heroTitle = new Label { Text = "Welcome back", AutoSize = true, Font = new Font("Segoe UI Semibold", 24F), Location = new Point(24, 20) };
        statusLabel.Text = "Checking launcher status...";
        ...
        var actions = new FlowLayoutPanel { Dock = DockStyle.Top, Height = 105, Padding = new Padding(0, 18, 0, 0), WrapContents = false, BackColor = Color.Transparent };
        ConfigureButton(playButton, "PLAY", 140);
        ConfigureButton(updateButton, "UPDATE", 140);
        ConfigureButton(repairButton, "REPAIR", 140);
        ConfigureButton(stopButton, "STOP", 110);
        ConfigureButton(folderButton, "GAME FOLDER", 140);
        ConfigureButton(githubButton, "GITHUB", 120);
        actions.Controls.AddRange(new Control[] { playButton, updateButton, repairButton, stopButton, folderButton, githubButton });
        ...
        home.Controls.Add(progress);

        progressLabel.Dock = DockStyle.Top;
        progressLabel.Height = 34;
        progressLabel.Padding = new Padding(4, 8, 0, 0);
        home.Controls.Add(progressLabel);
    ...
    private static void ConfigureButton(Button button, string text, int width)
    {
        ...
        button.BackColor = Color.FromArgb(42, 37, 65);
    ...
    private void ShowSection(string section)
    {
        ...
        {
            Text = "Infinite Ascension — Settings",
            Width = 620,
            Height = 360,
        ...
        close.Click += (_, _) => { autoUpdate = auto.Checked; form.Close(); };
    ...
    private async Task InitializeAsync()
    {
        ...
        if (autoUpdate && remoteBuild > localBuild && localBuild > 0)
    ...
    private async Task RefreshManifestAsync()
    {
        try
        {
            ...
            serverLabel.Text = "Updater: online";
        ...
        {
            statusLabel.Text = "Updater unavailable: " + ex.Message;
            serverLabel.Text = "Updater: offline";
        }
    }
    ...
    private int ReadLocalBuild()
    {
        var candidates = new[]
        {
            Path.Combine(GameRoot, "build.json"),
            Path.Combine(GameRoot, "launcher_build.json")
        };
        foreach (var path in candidates)
        {
            ...
            try
            {
                ...
                if (doc.RootElement.TryGetProperty("build", out var build)) return build.GetInt32();
            }
            catch { }
        }
        return 0;
    ...
    private async Task PlayAsync()
    {
        ...
        {
            statusLabel.Text = "Failed to start game: " + ex.Message;
        }
    }
    ...
    private async Task UpdateAsync(bool manual)
    {
        ...
        try
        {
            ...
            await RefreshManifestAsync();
            if (!manual && remoteBuild <= localBuild)
            {
                statusLabel.Text = "Game is up to date.";
                return;
            }

            statusLabel.Text = "Downloading game update...";
            var work = Path.Combine(TempRoot, Guid.NewGuid().ToString("N"));
            Directory.CreateDirectory(work);
            var zipPath = Path.Combine(work, "game.zip");
            var staging = Path.Combine(InstallRoot, "Game.next");
            var rollback = Path.Combine(InstallRoot, "Game.rollback");

            // Worker manifest supplies the authoritative URL and SHA-256.
            using var manifestResponse = await http.GetAsync(ManifestUrl);
            manifestResponse.EnsureSuccessStatusCode();
            var manifestJson = await manifestResponse.Content.ReadAsStringAsync();
            using var manifest = JsonDocument.Parse(manifestJson);
            var root = manifest.RootElement;
            var gameUrl = root.GetProperty("game_url").GetString() ?? throw new InvalidOperationException("Manifest missing game_url.");
            var expectedSha = root.GetProperty("game_sha256").GetString()?.Trim().ToLowerInvariant() ?? throw new InvalidOperationException("Manifest missing game_sha256.");
            ...
            if (!CryptographicOperations.FixedTimeEquals(Convert.FromHexString(actualSha), Convert.FromHexString(expectedSha)))
                throw new InvalidOperationException("Game package SHA-256 mismatch.");
            ...
            File.WriteAllText(Path.Combine(GameRoot, "build.json"), JsonSerializer.Serialize(new { build = targetBuild, commit = remoteCommit, platform = "windows" }));
            if (Directory.Exists(rollback)) Directory.Delete(rollback, true);
            localBuild = targetBuild;
            buildLabel.Text = $"Build {localBuild} → {remoteBuild}";
    ...
    private async Task RepairAsync()
    {
        ...
        var bak = GameRoot + ".repair-backup";
        try
        {
            busy = true;
            if (Directory.Exists(bak)) Directory.Delete(bak, true);
            Directory.Move(GameRoot, bak);
            Directory.CreateDirectory(GameRoot);
            foreach (var file in Directory.EnumerateFiles(bak, "*", SearchOption.AllDirectories))
            {
                if (Path.GetFileName(file).Equals(GameExe, StringComparison.OrdinalIgnoreCase)) continue;
                var relative = Path.GetRelativePath(bak, file);
                var dest = Path.Combine(GameRoot, relative);
                Directory.CreateDirectory(Path.GetDirectoryName(dest)!);
                File.Copy(file, dest, true);
            }
            statusLabel.Text = "Repair completed.";
        }
        catch (Exception ex)
        {
    ...
    private void EnsureGamePayload(string root)
    {
        var direct = Path.Combine(root, GameExe);
        if (File.Exists(direct)) return;
        ...
        foreach (var path in Directory.EnumerateFileSystemEntries(sourceRoot, "*", SearchOption.TopDirectoryOnly))
        {
    ...
    private async Task DownloadAsync(string url, string target)
    {
        ...
        while ((read = await source.ReadAsync(buffer)) > 0)
        {
            ...
            if (total > 0)
            {
                ...
                progressLabel.Text = $"Downloading update… {percent}%";
            }
        }
    }
    ...
    {
        Directory.CreateDirectory(GameRoot);
        Process.Start(new ProcessStartInfo("explorer.exe", GameRoot) { UseShellExecute = true });
    }

    private void OpenLogs()
    {
        var path = Path.Combine(InstallRoot, "launcher.log");
        if (!File.Exists(path)) File.WriteAllText(path, "Launcher log initialized.\n");
